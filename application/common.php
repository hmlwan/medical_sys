...
function get_number($number,$num){
...
function cert_api_1($data){
    ...
    $path = "/phonethree";
    $method = "GET";
    $appcode = "5fcad04d9d6742d693a400b791608b8c";
    $headers = array();
    array_push($headers, "Authorization:APPCODE " . $appcode);
    $querys = "idcard={$idcard}&phone={$phone}&realname={$real_name}";
    $bodys = "";
...
//    return true;
    ...
    $headers = array();
    array_push($headers, "Authorization:APPCODE " . $appcode);
    $querys = "bankcard={$card}&idcard={$idcard}&mobile={$phone}&name={$real_name}";
    $bodys = "";
    $url = $host . $path . "?" . $querys;
...
//    curl_setopt($curl, CURLOPT_HEADER, true);
    ...
    $out_put = curl_exec($curl);
    $rsp = json_decode($out_put,true);
    if(!$out_put || $rsp['code'] != 200 || $rsp['data']['result'] != 0){
        return false;
    }
    return true;
}
 ...
 * 火车票查询
 */
function train_api($data){
//    return array();
    $date = $data['date']; //可选 时间 2019-11-21
    $end = $data['end']; //必选 到达
    $start = $data['start']; //必选 出发
...
//    curl_setopt($curl, CURLOPT_HEADER, true);
    ...
    return $rsp['result']?$rsp['result']:array();
}
function get_week_name($times){
    $weekarray=array("日","一","二","三","四","五","六");
    $week = '星期'.$weekarray[date("w",$times)];
    return $week;
}
/**
 *  post数据
...
function curlPost($postFields,$url){
...
function _insert_array($array,$key,$arr){
...
function starReplace($name, $num = 0)
{
    ...
    if (in_array($surname, $doubleSurname)) {
        $name =  str_repeat('*', 2).mb_substr($name, 2, mb_strlen($name, 'UTF-8') ) ;
    } else {
        $name =  str_repeat('*', 1).mb_substr($name, 1, mb_strlen($name, 'UTF-8') ) ;
        ...
        while(($file = readdir($handle)) !== false) {
            if($file != ".." && $file != ".") { //排除根目录；
                if(is_dir($dir."/".$file)) { //如果是子文件夹，就进行递归
                    $files[$file] = array_merge( $files , get_all_files($dir."/".$file) ) ;
                } else { //不然就将文件的名字存入数组；
                    $files[] = $file;
                }
